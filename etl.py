...
def process_log_file(cur, filepath):
    ...
    dic_combin = dict(zip(column_labels, time_data))
    time_df = pd.DataFrame(data=dic_combin, columns=column_labels)
    ...
    for index, row in df.iterrows():
        ...
        songplay_data = (row.userId, songid,artistid, row.sessionId, 
                         pd.to_datetime(row.ts, unit='ms'), row.level, 
...
def process_data(cur, conn, filepath, func):
    ...
    Arguments:
        ...
        filepath: log data file path to be processed.
